...
| `tags`    | No       | JSON array (or comma list) of tags shown as pills in the cover header, in the given order (the player page aggregates them from the member mixes, most-used first); the first 6 show with a "+N more" pill revealing the rest; clicks emit `tagclick` |
